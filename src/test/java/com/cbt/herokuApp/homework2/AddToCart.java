...
public class AddToCart {
    ...
    public void test(){
       ...
       // System.out.println("Number of descriptions: " + descriptions.size());
        ...
        List<String> parsedPrices = BrowserWait.getTextFromWebElements(prices).parallelStream().map(p -> p.replace("\n",".")).collect(Collectors.toList());

        List<String> parsedDescriptions = BrowserWait.getTextFromWebElements(prices);
        parsedDescriptions.removeIf(String::isEmpty);//removes empty String
        ...
        System.out.println("Description: " + BrowserWait.getTextFromWebElements(descriptions));
    ...
    static {
        Runtime.getRuntime().addShutdownHook(new Thread(BrowserFactory::closeDriver));
    }
}
